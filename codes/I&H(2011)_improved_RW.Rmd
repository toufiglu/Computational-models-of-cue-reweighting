...
train$Cues<-as.character(train$Cues)
train$Outcomes<-as.character(train$Outcomes)
f0230.neutral<-mat.or.vec(maxRep,2)
f0290.neutral<-mat.or.vec(maxRep,2)
...
for (nRep in 1:maxRep)
{
    ...
    f0230.neutral[nRep,]<-w["normalised.f0199",c("b","p")]
    f0290.neutral[nRep,]<-w["normalised.f0269",c("b","p")]
...
#Calculating the mean activation of p and b across replications
#Putting it into the plot data
plot.data$mean.act.p_neutral<-apply(act.p,1,"mean")
plot.data$sd.act.p_neutral<-apply(act.p,1,"sd")
plot.data$mean.act.b_neutral<-apply(act.b,1,"mean")
plot.data$sd.act.b_neutral<-apply(act.b,1,"sd")
```
...
train$Cues<-as.character(train$Cues)
train$Outcomes<-as.character(train$Outcomes)
f0230.reversed<-mat.or.vec(maxRep,2)
f0290.reversed<-mat.or.vec(maxRep,2)
...
for (nRep in 1:maxRep)
{
    ...
    f0230.reversed[nRep,]<-w["normalised.f0199",c("b","p")]
    f0290.reversed[nRep,]<-w["normalised.f0269",c("b","p")]
...
#Calculating the mean activation of p and b across replications
#Putting it into the plot data
plot.data$mean.act.p_reversed<-apply(act.p,1,"mean")
plot.data$sd.act.p_reversed<-apply(act.p,1,"sd")
plot.data$mean.act.b_reversed<-apply(act.b,1,"mean")
plot.data$sd.act.b_reversed<-apply(act.b,1,"sd")
```
...
plot.data$prob.reversed<-plot.data$mean.act.p_reversed/(plot.data$mean.act.p_reversed+plot.data$mean.act.b_reversed)
plot.data$prob.neutral<-plot.data$mean.act.p_neutral/(plot.data$mean.act.p_neutral+plot.data$mean.act.b_neutral)

ggfodata <- plot.data %>%
  group_by(F0) %>%
  summarise_at(grep("prob", names(plot.data), value = TRUE), mean) %>%
  gather(value = "meanfo", key = "condition", -F0) %>%
  mutate(F0 = gsub("f0", "", F0))

gf0 <- ggfodata %>%
  ggplot(aes(x = condition, y = meanfo, group = F0)) + 
  geom_point(aes(shape = F0), size = 3) +
  theme(legend.text = element_text (size = 12),
        legend.title = element_text(size = 12, face = "bold"),
        plot.title = element_text(size = 12),
